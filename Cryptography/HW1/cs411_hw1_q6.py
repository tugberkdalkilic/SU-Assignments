...
def main():
    ...
    ciphertext_unfiltered = "JYPR LTAXTNM, SOGELBCONN IL TNY ZER TU VGIELOUCCX, EXEPY LABUH"

    ciphertext = ''.join(x.lower()
                         for x in ciphertext_unfiltered if x.isalpha())

    key_length = get_key_length(ciphertext)
    print("Key length is most likely {}".format(key_length))

    # As we understand that it is a message from our professor
    # Professor can start a message with "Dear Students"
    # As key length is possibly 6

    # to find the key, i will subtract first 6 cipher letter from "dear st" to find the key
    # after i find the key, i will send it to decryption function

    plain_version = "dearst"
    cipher_version = "jyprlt"
    key= "gupata"
